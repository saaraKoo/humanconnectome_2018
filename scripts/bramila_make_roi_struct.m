...
% MY_BRAMILA_MAKEROISTRUCT - Computes the ROI (Regions of interest) struct
% variable, used to define regions of interests in the BraMiLa and FUNPSY
% toolboxes. This is a personal copy of Onerva, in order to use a general
% space2MNI transform instead of the 2 mm one.
%
%   - Usage: 
...
%       cfg.res = spatial resolution in mm, default 2
%   - Output:
...
% Copy created by Onerva 2014-07-24
...
if isfield(cfg,'roimask')
    ...
    for r=1:size(roicoo,1)
        temp=roicoo(r,:);
        rois(r).centroid = my_MNI2space([temp(1), temp(2), temp(3)], cfg.res);
    end
...
for n=1:length(rois);
    ...
    rois(n).centroidMNI = my_space2MNI([rois(n).centroid(1),rois(n).centroid(2),rois(n).centroid(3)], cfg.res);
